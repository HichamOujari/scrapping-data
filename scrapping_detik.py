...
def scrap_detik_url(query):
    ...
    try:
            source = requests.get("https://www.detik.com/search/searchall?query="+query[0]).text
            time.sleep(5)
            soup = BeautifulSoup(source, 'lxml')
            ...
            for i in range(2,10):
                ...
                source = requests.get("https://www.detik.com/search/searchall?query="+query[0]+"&sortby=time&page={}".format(i)).text
                time.sleep(5)
                soup = BeautifulSoup(source, 'lxml')
